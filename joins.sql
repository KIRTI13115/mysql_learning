 ...
 -- FULL OUTER JOINS
 -- JOIns matching rows in both tables + non matching rows from table A to table b + non matching rows from table B to table A 
 
-- While FULL JOIN (or FULL OUTER JOIN) is standard SQL, MySQL does not natively support it.

-- CROSS JOIN
-- used to match each row from table A to each row in Table B 
